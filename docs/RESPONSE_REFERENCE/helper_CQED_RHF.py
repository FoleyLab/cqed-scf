...
def cqed_rhf(lambda_vector, molecule_string, psi4_options_dict):
    ...
    # create d(1) d(2) tensor product in same shape as 2-e integrals 
    #d_tp = np.einsum("pq,rs->pqrs", l_dot_mu_el, l_dot_mu_el)

    # add to 2-e integrals to get the full 2-e contribution to the Fock matrix
    #I += d_tp
    # Gemini recommended this alternative to avoid creating the large 4-index tensor d_tp, but it is not clear to me that this is more efficient than just creating d_tp and adding it to I
    I += l_dot_mu_el[:, :, np.newaxis, np.newaxis] * l_dot_mu_el[np.newaxis, np.newaxis, :, :]
    print("Finished building integrals and initial guess. Starting SCF iterations...\n")
    ...
    for SCF_ITER in range(1, maxiter + 1):
        ...
        # try tensor product 
        #Mtp = np.einsum("pqrs,rs->pq", d_tp, D)
        #Ntp = np.einsum("prqs,rs->pq", d_tp, D)

        # compare M and N to Mtp and Ntp to check that the tensor product gives the same result as the explicit sums
        #diff_M = np.mean((M - Mtp) ** 2) ** 0.5
        #diff_N = np.mean((N - Ntp) ** 2) ** 0.5
        #print(diff_M, diff_N)
