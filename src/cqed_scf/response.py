...
class QEDCIS(CQEDResponse):
    ...
    def kernel(self, nroots: Optional[int] = None) -> QEDCISResults:
        """Build and fully diagonalize.  Hermitian only, by design."""
        ...
        if nroots is not None:
            ...
            eigenvalues = eigenvalues[:nroots]
            eigenvectors = eigenvectors[:, :nroots]
    ...
    def excitation_energies(self, nroots: Optional[int] = None) -> np.ndarray:
        return self.kernel(nroots=nroots).excitation_energies
